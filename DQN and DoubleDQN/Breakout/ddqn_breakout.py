#Training an agent to play Atari Game - Breakout using DoubleDQN
...
output_dir = 'model_output/breakout-ddqn'
...
class DQNAgent:
    ...
    def replay(self):
        ...
        actions_max = np.argmax(Q_values_next, axis=1)
        ...
        targets_next = np.asarray([[targets_next[j][actions_max[j]] for i in range(targets_next.shape[1])] for j in range(targets_next.shape[0])])
        
        targets_next = np.repeat(np.reshape(rewards, (-1, 1)), self.action_size, axis=1)  + self.gamma * \
        np.reshape(1-done, (-1, 1)) * targets_next
...
while frames_trained_for < total_frames_to_train_for:
    ...
    for time in range(max_steps_in_episode):
        ...
        if done:
            sys.stdout = open("progress_ddqn_breakout.txt", "a")
